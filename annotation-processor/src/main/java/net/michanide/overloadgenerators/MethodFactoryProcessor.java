...
@SupportedAnnotationTypes(OverGenAnnotationProcessors.COMPUTER_METHOD_FACTORY_ANNOTATION_CLASSNAME)
@SupportedOptions(OverGenAnnotationProcessors.MODULE_OPTION)
public class MethodFactoryProcessor extends AbstractProcessor {
    private String mekModule;
    private final ClassName factoryRegistry = ClassName.get(OverGenAnnotationProcessors.COMPUTER_INTEGRATION_PACKAGE, "FactoryRegistry");
    private final ClassName methodRegistryInterface = ClassName.get(OverGenAnnotationProcessors.COMPUTER_INTEGRATION_PACKAGE, "IComputerMethodRegistry");
    ...
    public synchronized void init(ProcessingEnvironment processingEnv) {
        ...
        mekModule = "overloadgenerators";
    }

    @Override
    public boolean process(Set<? extends TypeElement> annotatedTypes, RoundEnvironment roundEnvironment) {
        TypeMirror methodFactoryType = processingEnv.getElementUtils().getTypeElement(OverGenAnnotationProcessors.COMPUTER_METHOD_FACTORY_ANNOTATION_CLASSNAME).asType();
        ...
        if (!registryType.originatingElements.isEmpty()) {
            ...
            String packageName = "net.michanide.overloadgenerators.generated";
            try {
    ...
    private void addHandlerToRegistry(TypeElement handledType, ClassName factoryClassName) {
        ...
        do {
            ...
            if (clazz.canonicalName().startsWith("mekanism") || clazz.canonicalName().startsWith("net.michanide.overloadgenerators")) {
